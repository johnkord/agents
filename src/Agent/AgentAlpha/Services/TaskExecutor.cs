...
/// <summary>
/// Implementation of task execution orchestration with tool approval integration
/// </summary>
public class TaskExecutor : ITaskExecutor
{
    ...
    public async Task ExecuteAsync(string task)
    {
        ...
        try
        {
            // Step 1: Initialize tool approval system
            InitializeToolApprovalSystem();

            // Step 2: Connect to MCP Server
            await ConnectToMcpServerAsync();

            // Step 3: Discover and filter tools
    ...
    private void InitializeToolApprovalSystem()
    {
        try
        {
            var approvalBackend = _config.ToolApproval.CreateBackend();
            ToolApprovalManager.Instance.SetApprovalBackend(approvalBackend);
            
            _logger.LogInformation("Tool approval system initialized with {BackendType} backend", 
                _config.ToolApproval.BackendType);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Failed to initialize tool approval system");
            throw;
        }
    }
    ...
    private void InitializeConversation(string task)
    {
        var systemPrompt = """
            ...
            Note: Some tools may require approval before execution for security reasons.
            """;
