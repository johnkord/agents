...
/// <summary>
/// Implementation of tool discovery, validation, and schema management with approval support
/// </summary>
public class ToolManager : IToolManager
{
    ...
    public async Task<string> ExecuteToolAsync(IConnectionManager connection, string toolName, Dictionary<string, object?> arguments)
    {
        try
        {
            ...
            var approved = await ToolApprovalManager.Instance.EnsureApprovedAsync(toolName, arguments);
            if (!approved)
            {
                _logger.LogWarning("Tool {ToolName} execution denied by approval system", toolName);
                return $"Tool '{toolName}' execution denied by approval system.";
            }
